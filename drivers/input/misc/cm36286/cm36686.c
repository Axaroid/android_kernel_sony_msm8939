...
{
    struct cm36686_info *lpi = lp_info;
    uint16_t ps_conf1 = 0, ps_conf3 = 0;

    _cm36686_I2C_Read_Word(lpi->slave_addr, PS_CONF1, &ps_conf1);
    _cm36686_I2C_Read_Word(lpi->slave_addr, PS_CONF3, &ps_conf3);

    return sprintf(buf, "PS_CONF1 REG:0x%04x var:0x%04x, PS_CONF3 REG:0x%04x var:0x%04x\n",
            ps_conf1, lpi->ps_conf1_val, ps_conf3, lpi->ps_conf3_val);
}
